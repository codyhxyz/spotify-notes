...
- **Old notes get their song titles and covers back.** 171 notes imported before the app started storing track details had no name, artist, or album art, so My Library showed them as "Unknown track" on a blank card. `/api/notes` now takes a `PATCH` that fills in just those details for notes you already have — it never touches the note text and never changes when the note was last edited, so nothing reshuffles in your Library. The Fastpotify Notes desktop app looks the songs up on Spotify and sends them back through it, and the fix shows up everywhere you read your notes.
